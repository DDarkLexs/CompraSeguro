...
const UpdateProductScreen: React.FC<
  NativeStackScreenProps<StackScreen, Routes.UPDATE_PRODUCT>
> = ({ navigation, route }): React.JSX.Element => {
  const theme = useTheme();
  const [produto, setProduto] = React.useState<IProdutos>(route.params);
  const [loading, setLoding] = React.useState(false);

  const registar = async () => {
    try {
      setLoding(true);
      await repo.updateProduto(route.params.id_produto, produto);

      Alert.alert("Sucesso", `Produto ${produto.nome} foi atualizado com sucesso`, [
        {
          text: "Ok",
